...
Applies a Gaussian blur of the given sigma to soften mask edges.

**Parameters**:

- `sigma`: standard deviation in pixels (typical: 0.5 - 3.5)

**Implementation**: `internal/mask/blurkernel` picks the kernel from sigma.
Below sigma 4 it convolves a true Gaussian directly; above it, a 3-pass box
blur with three distinct radii. The split exists because a box approximation
cannot represent sigma below ~0.8 — the narrowest non-trivial box is already
sigma 0.82 — and most of this renderer's sigmas are in that range.

Both passes run through one kernel that has an AVX2 implementation
(`blurkernel/asm/amd64`), selected at run time and falling back to portable Go
on other architectures, under `-tags purego`, and on js/wasm.

`GaussianBlur`, the `gift`-based reference, now lives in a test file. It is the
oracle the fast kernels are measured against and is not in the render path.
...
**Implementation**: Wrapper around `BoxBlurSigma` with a small sigma.
...
blurred := BoxBlurSigma(mask, 2.45)
noise := GeneratePerlinNoise(256, 256, 30.0, seed)
noisy := ApplyNoiseToMask(blurred, noise, 0.28)
thresholded := ApplyThreshold(noisy, 50)
final := AntialiasEdges(thresholded, 1.41)

// DefaultParams (internal/watercolor/processor.go):
// - blur sigma: 2.45 (per-layer 1.41 for rivers, 2.45 elsewhere)
// - noise strength: 0.28
// - threshold: 50
// - antialias sigma: 1.41
// - land shade sigma: 7.48
//
// These are blur widths in pixels. They carry odd-looking decimals because they
// were derived from what the previous blur was really doing, which was about
// twice its nominal sigma — see the comment on DefaultParams.
```
...
2. ✅ **TestBlurAccuracyVsGaussian**: Blur accuracy against a true Gaussian, per sigma
